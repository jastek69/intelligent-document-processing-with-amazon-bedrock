...
class MCPTester:
    ...
    def test_connectivity(self) -> bool:
        ...
        try:
            response = requests.post(self.config["mcp_url"], headers=self.config["headers"], json=init_request, timeout=30)
    ...
    def test_tools_listing(self) -> bool:
        ...
        try:
            ...
            response = requests.post(self.config["mcp_url"], headers=self.config["headers"], json=tools_request, timeout=30)
    ...
    def test_aws_integration(self) -> bool:
        ...
        try:
            ...
            response = requests.post(self.config["mcp_url"], headers=self.config["headers"], json=bucket_request, timeout=60)
    ...
    def test_document_extraction(self) -> bool:
        ...
        try:
            ...
            response = requests.post(self.config["mcp_url"], headers=self.config["headers"], json=extract_request, timeout=180)
...
Examples:
  ...
  python test_mcp_server.py --extraction-only  # Run only document extraction test
    ...
    parser.add_argument(
        "--full", 
        action="store_true", 
        help="Run full test suite including document extraction"
    )
    
    parser.add_argument(
        "--extraction-only", 
        action="store_true", 
        help="Run only document extraction test"
    )

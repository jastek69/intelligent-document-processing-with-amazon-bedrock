...
def get_existing_cognito_config():
    ...
    try:
        ...
        # Get Client ID
        client_id_param = ssm_client.get_parameter(
            Name=f"/{stack_name}/ecs/CLIENT_ID"
        )
        ...
        # Get Cognito Domain
        domain_param = ssm_client.get_parameter(
            Name=f"/{stack_name}/ecs/COGNITO_DOMAIN"
        )
        ...
        print(f"✅ Found existing Cognito configuration:")
...
def get_existing_infrastructure_config():
    ...
    try:
        ...
        try:
            # List state machines and find the idp-bedrock one
        ...
        try:
            # List buckets and find the idp-bedrock one
        ...
        # If we couldn't find them, try the expected naming pattern
        if not state_machine_arn:
            ...
            try:
                ...
                state_machine_arn = expected_sm_arn
                print(f"✅ Found Step Functions (expected name): {state_machine_arn}")
            except Exception as e:
        ...
        if not bucket_name:
            ...
            try:
                ...
                print(f"✅ Found S3 bucket (expected name): {bucket_name}")
            except Exception as e:
...
def get_existing_user_credentials(cognito_config, username=None, password=None):
    ...
    If username/password not provided, will prompt for them
    """
    try:
        cognito_client = boto3.client('cognito-idp', region_name=cognito_config['region'])
        user_pool_id = cognito_config['user_pool_id']
        
        # If no username provided, list existing users and let user choose
        if not username:
            ...
            try:
                ...
                # For now, use the first user (typically the main user from config.yml)
        ...
        # If no password provided, we'll need to prompt for it or use a default
        if not password:
            ...
            # For demo purposes, we'll try a common pattern, but in production this should be input
        ...
        try:
            ...
            print(f"✅ Generated bearer token for MCP access")
...
def create_agentcore_role(agent_name="tabulate-mcp"):
    ...
    try:
        iam_client.put_role_policy(
            ...
            PolicyName="TabulateAgentCorePolicy",
            RoleName=agentcore_role_name
        )
    ...
    try:
        ...
        try:
            ...
            secrets_client.update_secret(
                SecretId='tabulate-mcp/cognito/credentials',
                SecretString=json.dumps(mcp_credentials)
            )
